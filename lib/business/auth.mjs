...
class Auth {
    ...
    async sendEmail({ userInfo, template, subject, replaceTags }) {
        ...
        const htmlContent = replaceTags ? template.replace('${OTP}', userInfo.tags.OTP).replace('${Username}', userInfo.UserName) : template;
        const mailOptions = {
    ...
    async recoverPassword({ req, res, template, appName }) {
        ...
        if (email) {
            const userData = await BusinessBase.businessObject.sql.query(`SELECT * FROM Security_User WHERE EmailAddress = '${email}' AND IsDeleted = 0`);
            if (!userData[0]) {
                response.success = false
                response.info = "No user found with this email"
                ...
                if (!tokenKey) {
                        ...
                        await this.sendEmail({ userInfo, template: template[0], subject: `${appName} password change activation token`, replaceTags: true });
                        response = {
                            success: true,
                            info: 'Token has been sent'
                        }
                }
                else {
                    ...
                    if (user[0] && newPassword && email) {
                        ...
                        await this.sendEmail({userInfo, template: template[1], subject: `${appName} | Password Changed Successfully!`, replaceTags: false});
                        response = {
                            data: [],
                            success: true,
