...
class User extends Auth {
    ...
    static async login({ username, password, scopeId, withoutPassword = false, dSql }) {
        ...
        const result = await dSql.execute({
            query: 'Security_Login2',
            parameters
        });
        const [[userInfo], roles, modules, tags] = result.recordsets;
        ...
        for (const tag of tags) {
            try {
                ...
                // Check if parsed value is an object/array, otherwise keep the original string
                if (typeof parsedValue === "object" || Array.isArray(parsedValue)) {
                    tagDictionary[tag.Key] = parsedValue;
                } else {
                    tagDictionary[tag.Key] = tag.Value; // Keep as string if it's a number or primitive
                }
            } catch (e) {
